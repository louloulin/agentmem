...
mod tests {
    ...
    async fn test_add_and_get_vector() {
        ...
        assert_eq!(retrieved_data.metadata.get("category").unwrap(), "cache_test");
    ...
    async fn test_cache_warm_and_cleanup() {
        ...
        // 缓存预热
        let warmed = store.warm_cache(vec!["warm1".to_string(), "warm2".to_string(), "nonexistent".to_string()]).await.unwrap();
    ...
    async fn test_ttl_operations() {
        ...
        // 批量设置 TTL
        let result = store.set_ttl_batch(vec!["ttl1".to_string(), "ttl2".to_string()], 3600).await;
        assert!(result.is_ok());
    }
