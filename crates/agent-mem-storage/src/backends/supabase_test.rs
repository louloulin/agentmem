...
#[cfg(test)]
mod tests {
    use super::super::supabase::{SupabaseConfig, SupabaseStore, SimilarityFunction};
    ...
    async fn test_add_and_get_vector() {
        ...
        assert_eq!(retrieved_data.metadata.get("source").unwrap(), "supabase_test");
    }
    ...
    async fn test_postgresql_features() {
        ...
        assert_eq!(retrieved.metadata.get("title").unwrap(), "PostgreSQL Document");
        assert_eq!(retrieved.metadata.get("author").unwrap(), "Jane Doe");
        assert_eq!(retrieved.metadata.get("tags").unwrap(), "postgresql,vector,search");
