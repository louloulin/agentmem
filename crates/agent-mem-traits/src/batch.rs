...
use crate::{EnhancedAddRequest, BatchResult, MemorySearchResult, Result, HealthStatus};
...
impl MemoryUpdate {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.id.trim().is_empty() {
            return Err(crate::AgentMemError::ValidationError("Empty memory ID".to_string()));
        }
        
        if let Some(content) = &self.content {
            if content.trim().is_empty() {
                return Err(crate::AgentMemError::ValidationError("Empty content".to_string()));
            }
        }
...
pub trait AdvancedSearch: Send + Sync {
    ...
    async fn search_similar(&self, memory_id: &str, limit: usize) -> Result<Vec<MemorySearchResult>>;
...
pub trait ConfigurationProvider: Send + Sync {
    ...
    async fn watch_config(&self, key: &str) -> Result<std::sync::mpsc::Receiver<Option<serde_json::Value>>>;
}
