...
impl LocalTestProvider {
    ...
    fn create_response_templates() -> HashMap<String, String> {
        ...
        templates.insert(
            "default".to_string(),
            "这是一个来自本地测试 LLM 的响应。我理解了您的请求，并将尽力提供有用的信息。".to_string(),
        );
    ...
    /// 生成模拟的使用统计
    fn generate_usage_stats(&self, input_tokens: usize, output_tokens: usize) -> HashMap<String, serde_json::Value> {
        let mut usage = HashMap::new();
        usage.insert("prompt_tokens".to_string(), serde_json::Value::Number(input_tokens.into()));
        usage.insert("completion_tokens".to_string(), serde_json::Value::Number(output_tokens.into()));
        usage.insert("total_tokens".to_string(), serde_json::Value::Number((input_tokens + output_tokens).into()));
        usage
    }
...
impl LLMProvider for LocalTestProvider {
    async fn generate(&self, messages: &[Message]) -> Result<String> {
        ...
        let limited_response = if response.len() > self.local_config.max_tokens as usize {
            format!("{}...", &response[..self.local_config.max_tokens as usize - 3])
        } else {
            response
        };
        
        if self.local_config.verbose {
            debug!("本地测试 LLM 生成响应完成，长度: {}", limited_response.len());
        }
...
impl LocalTestProvider {
    /// 生成带元数据的响应（辅助方法）
    pub async fn generate_with_metadata(&self, messages: &[Message]) -> Result<(String, HashMap<String, serde_json::Value>)> {
        let response = self.generate(messages).await?;

        // 计算 token 使用量
        let input_tokens: usize = messages.iter()
        ...
        metadata.insert("model".to_string(), serde_json::Value::String(self.local_config.model_name.clone()));
        metadata.insert("provider".to_string(), serde_json::Value::String("local_test".to_string()));
        metadata.insert("temperature".to_string(), serde_json::Value::Number(
            serde_json::Number::from_f64(self.local_config.temperature as f64).unwrap_or_else(|| serde_json::Number::from(0))
        ));
        metadata.insert("max_tokens".to_string(), serde_json::Value::Number(self.local_config.max_tokens.into()));
        metadata.insert("usage".to_string(), serde_json::Value::Object(
