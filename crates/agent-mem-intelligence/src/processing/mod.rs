...
impl MemoryProcessor {
    ...
    pub fn new(config: ProcessingConfig) -> Self {
        ...
        let adaptive_manager = AdaptiveMemoryManager::new(
            config.max_memories_per_scope,
    ...
    pub async fn process_memories(&mut self, memories: &mut Vec<Memory>) -> Result<ProcessingStats> {
        ...
        if self.config.enable_importance_scoring {
            let updated = self.importance_scorer.update_importance_scores(memories).await?;
    ...
    pub fn update_config(&mut self, config: ProcessingConfig) {
        ...
        self.consolidator.update_threshold(config.consolidation_threshold);
        self.importance_scorer.update_decay_rate(config.importance_decay_rate);
        self.adaptive_manager.update_limits(
            config.max_memories_per_scope,
            config.retention_period,
        );
    }
