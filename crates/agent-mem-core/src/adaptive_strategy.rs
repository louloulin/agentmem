...
impl AdaptiveStrategyManager {
    ...
    pub async fn recommend_strategy(
        ...
        if let Some(performance) = recent_performance {
            self.record_performance(self.current_strategy.clone(), performance).await?;
        }
    ...
    async fn record_performance(
        ...
        let perf = self.strategy_performance.entry(strategy.clone()).or_insert_with(|| {
            StrategyPerformance {
        ...
        self.performance_history.push_back((Utc::now(), strategy, performance));
    ...
    async fn predict_optimal_strategy(
        ...
        let mut alternatives: Vec<_> = strategy_scores
            ...
            .collect();
        alternatives.sort_by(|(_, a), (_, b)| b.partial_cmp(a).unwrap_or(std::cmp::Ordering::Equal));
    ...
    fn calculate_context_similarity(&self, _context: &ScoringContext, _strategy: &MemoryStrategy) -> f64 {
    ...
    async fn evaluate_adaptation_need(&mut self) -> Result<()> {
        ...
        if current_performance < self.config.performance_threshold {
            ...
            if let Some((better_strategy, better_performance)) = self.strategy_performance
                .iter()
    ...
    fn initialize_default_strategies(&mut self) {
        ...
        for strategy in strategies {
            self.strategy_performance.insert(strategy.clone(), StrategyPerformance {
                strategy: strategy.clone(),
    ...
    pub fn get_adaptation_log(&self) -> &VecDeque<(DateTime<Utc>, MemoryStrategy, MemoryStrategy, String)> {
        &self.adaptation_log
    }
}
...
mod tests {
    ...
    async fn test_strategy_recommendation() {
        ...
        match recommendation.recommended_strategy {
            MemoryStrategy::Conservative | MemoryStrategy::Aggressive |
            MemoryStrategy::Balanced | MemoryStrategy::ContextAware |
            MemoryStrategy::UserCentric | MemoryStrategy::TaskOriented => {
                // Valid strategy
            }
        }
    ...
    async fn test_performance_recording() {
        ...
        manager.record_performance(MemoryStrategy::Balanced, 0.8).await.unwrap();
        
        let performance = manager.strategy_performance.get(&MemoryStrategy::Balanced).unwrap();
    ...
    fn test_strategy_parameters() {
        ...
        assert_eq!(params.conflict_resolution, ConflictResolutionStrategy::KeepBoth);
        assert_eq!(params.importance_threshold, 0.3);

        let params = manager.get_strategy_parameters(&MemoryStrategy::Aggressive);
        assert_eq!(params.conflict_resolution, ConflictResolutionStrategy::ImportanceBased);
