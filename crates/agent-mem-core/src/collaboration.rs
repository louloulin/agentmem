...
impl SharedMemoryPool {
    ...
    pub async fn add_shared_memory(
        ...
        if memories.len() >= self.config.max_shared_pool_size {
            return Err(AgentMemError::validation_error("Shared memory pool is full"));
        }
    ...
    pub async fn get_shared_memory(
        ...
        // 检查权限
        if !self.check_permission(memory_id, requesting_agent, &AccessType::Read).await? {
            self.record_access(
    ...
    pub async fn get_access_statistics(&self) -> Result<AccessStatistics> {
        ...
        let unique_agents = history.iter().map(|r| &r.agent_id).collect::<HashSet<_>>().len();
        ...
        for record in history.iter() {
            *access_by_memory.entry(record.memory_id.clone()).or_insert(0) += 1;
        }
...
pub enum PolicyAction {
    ...
    RequireApproval,
    Log,
    Throttle { max_requests: u32, window_seconds: u32 },
}
...
impl PermissionManager {
    ...
    pub async fn get_agent_permission(&self, agent_id: &str) -> Result<AgentPermissionLevel> {
        ...
        Ok(permissions.get(agent_id).cloned().unwrap_or(AgentPermissionLevel::ReadOnly))
    }
    ...
    pub async fn evaluate_permission(
        ...
        for policy in policies.iter() {
            if self.evaluate_condition(&policy.condition, agent_id, memory_id, access_type, context)? {
...
impl ConflictResolver {
    ...
    pub async fn resolve_conflict(&self, conflict_id: &str) -> Result<ConflictResolution> {
        ...
        let resolution = match &self.strategy {
            ...
            ConflictResolutionStrategy::VotingBased => {
                self.resolve_voting_based(conflict).await?
            },
            ConflictResolutionStrategy::MergeBased => {
                self.resolve_merge_based(conflict).await?
            },
        };
    ...
    /// 最后写入者获胜解决策略
    async fn resolve_last_writer_wins(&self, conflict: &MemoryConflict) -> Result<ConflictResolution> {
        let latest_version = conflict.conflicting_versions
            .iter()
    ...
    /// 基于重要性解决策略
    async fn resolve_importance_based(&self, conflict: &MemoryConflict) -> Result<ConflictResolution> {
        let most_important = conflict.conflicting_versions
            .iter()
    ...
    /// 基于访问频率解决策略
    async fn resolve_access_frequency_based(&self, conflict: &MemoryConflict) -> Result<ConflictResolution> {
        let most_accessed = conflict.conflicting_versions
            .iter()
    ...
    async fn resolve_voting_based(&self, conflict: &MemoryConflict) -> Result<ConflictResolution> {
        ...
        for vote in &votes {
            ...
            *vote_scores.entry(vote.preferred_version.clone()).or_insert(0.0) +=
        ...
        let winning_content = conflict.conflicting_versions
            .iter()
    ...
    async fn resolve_merge_based(&self, conflict: &MemoryConflict) -> Result<ConflictResolution> {
        // 简化的合并策略：连接所有版本的内容
        let merged_content = conflict.conflicting_versions
            .iter()
    ...
    pub async fn get_conflict_statistics(&self) -> Result<ConflictStatistics> {
        ...
        for conflict in conflicts.values() {
            if let Some(resolution) = &conflict.resolution {
                *resolution_by_type.entry(resolution.resolution_type.clone()).or_insert(0) += 1;
            }
        }
        ...
        Ok(ConflictStatistics {
            ...
            pending_conflicts,
            resolution_by_type,
            average_resolution_time_seconds: self.calculate_average_resolution_time(&conflicts).await,
        })
    }

    /// 计算平均解决时间
    async fn calculate_average_resolution_time(&self, conflicts: &HashMap<String, MemoryConflict>) -> f64 {
        let resolved_conflicts: Vec<_> = conflicts.values()
            .filter(|c| matches!(c.status, ConflictStatus::Resolved))
            .collect();
        ...
        let total_time: i64 = resolved_conflicts.iter()
            .filter_map(|c| c.resolution.as_ref().map(|r| (r.resolution_time - c.detected_at).num_seconds()))
            .sum();
...
impl VotingSystem {
    ...
    pub async fn add_vote(&self, vote: Vote) -> Result<()> {
        ...
        votes.entry(vote.conflict_id.clone()).or_insert_with(Vec::new).push(vote);
        Ok(())
    }
...
impl KnowledgePropagator {
    ...
    pub async fn propagate_knowledge(
        ...
        let knowledge = knowledge_base.get(knowledge_id)
        ...
        for target_agent in &target_agents {
            // 检查订阅关系
            if self.is_subscribed(target_agent, &knowledge.knowledge_type).await? {
                // 应用知识衰减
    ...
    pub async fn subscribe(
        ...
        subscriptions.entry(agent_id).or_insert_with(HashSet::new).insert(knowledge_type_str);
        Ok(())
    }
    ...
    pub async fn get_propagation_statistics(&self) -> Result<PropagationStatistics> {
        ...
        for knowledge in knowledge_base.values() {
            *knowledge_by_type.entry(knowledge.knowledge_type.clone()).or_insert(0) += 1;
        }

        // 按智能体统计传播次数
        let mut propagations_by_agent = HashMap::new();
        for record in history.iter() {
            *propagations_by_agent.entry(record.source_agent.clone()).or_insert(0) += 1;
        }
...
impl CollaborativeMemorySystem {
    ...
    pub fn new(config: CollaborationConfig) -> Self {
        ...
        Self {
            ...
            conflict_resolver: Arc::new(ConflictResolver::new(config.conflict_resolution_strategy.clone())),
    ...
    /// 执行协作操作
    pub async fn execute_operation(&self, operation: CollaborationOperation) -> Result<OperationResult> {
        match operation {
            CollaborationOperation::ShareMemory { memory_id, target_agents, permission_level } => {
                self.share_memory(memory_id, target_agents, permission_level).await
            },
            CollaborationOperation::RequestAccess { memory_id, requesting_agent, access_type } => {
                self.request_access(memory_id, requesting_agent, access_type).await
            },
            CollaborationOperation::PropagateKnowledge { knowledge, target_agents } => {
                self.propagate_knowledge(knowledge, target_agents).await
            },
            CollaborationOperation::ResolveConflict { conflict_id, resolution } => {
    ...
    async fn share_memory(
        ...
        // 检查记忆是否存在
        let memory = self.shared_memory_pool.get_shared_memory(&memory_id, "system").await?;
        if memory.is_none() {
        ...
        for agent_id in &target_agents {
            self.permission_manager.set_agent_permission(
                agent_id.clone(),
    ...
    async fn request_access(
        ...
        let decision = self.permission_manager.evaluate_permission(
            &requesting_agent,
            &memory_id,
            &access_type,
            &context,
        ...
        if decision.allowed {
            // 执行实际访问
            let memory = self.shared_memory_pool.get_shared_memory(&memory_id, &requesting_agent).await?;
    ...
    async fn propagate_knowledge(
        ...
        // 添加知识到传播器
        self.knowledge_propagator.add_knowledge(knowledge.clone()).await?;

        // 执行传播
        let result = self.knowledge_propagator.propagate_knowledge(
            &knowledge.id,
            target_agents,
        ).await?;
    ...
    async fn resolve_conflict(
        ...
        let resolution = self.conflict_resolver.resolve_conflict(&conflict_id).await?;
    ...
    pub async fn get_system_statistics(&self) -> Result<CollaborationStatistics> {
        ...
        let propagation_stats = self.knowledge_propagator.get_propagation_statistics().await?;
