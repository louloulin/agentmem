...
pub async fn get_metrics(
    ...
    let episodic_count = stats.memories_by_type.get(&agent_mem_core::MemoryType::Episodic).unwrap_or(&0);
    let semantic_count = stats.memories_by_type.get(&agent_mem_core::MemoryType::Semantic).unwrap_or(&0);
    let procedural_count = stats.memories_by_type.get(&agent_mem_core::MemoryType::Procedural).unwrap_or(&0);
