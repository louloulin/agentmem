    ...
    async fn handle_response<T: DeserializeOwned>(&self, response: Response) -> ClientResult<T> {
        ...
        if status.is_success() {
            ...
            if let Ok(error_response) = serde_json::from_str::<ErrorResponse>(&body) {
                error!("Server error: {} - {}", error_response.code, error_response.message);
...
impl AgentMemClient {
    ...
    pub fn get_memory(&self, memory_id: &str) -> ClientResult<Memory> {
        self.runtime.block_on(self.async_client.get_memory(memory_id))
    }
    ...
    pub fn search_memories(
        ...
        self.runtime.block_on(self.async_client.search_memories(request))
    }
