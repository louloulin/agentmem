...
async fn test_memory_operations(client: &Client, base_url: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Adding memory: {}", serde_json::to_string_pretty(&memory_request)?);
    ...
    if response.status().is_success() {
        let body: serde_json::Value = response.json().await?;
        info!("Add memory response: {}", serde_json::to_string_pretty(&body)?);
        ...
        info!("Searching memories: {}", serde_json::to_string_pretty(&search_request)?);
        ...
        if search_response.status().is_success() {
            ...
            info!("Search response: {}", serde_json::to_string_pretty(&search_body)?);
        } else {
