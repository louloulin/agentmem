...
async fn main() -> Result<()> {
    ...
    let structured_message = Messages::Structured(Message {
        ...
        content: "I am a senior software engineer with expertise in Rust and distributed systems".to_string(),
        timestamp: None,
    });
    ...
    info!("🔍 Found {} memories related to programming:", search_results.len());
    for (i, memory) in search_results.iter().enumerate() {
        info!("  {}. ID: {}, Content: {}", i + 1, memory.id, memory.content);
    }

    // Test 5: Search for professional memories
    info!("🔍 Test 5: Searching for professional memories");
    let professional_results = client.search(
    ...
    info!("🔍 Found {} professional memories:", professional_results.len());
