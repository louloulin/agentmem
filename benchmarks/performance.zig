...
const AgentDatabase = @import("agent_db.zig").AgentDatabase;
const AgentState = @import("agent_db.zig").AgentState;
const StateType = @import("agent_db.zig").StateType;
...
pub fn main() !void {
    ...
    var db = AgentDatabase.init(allocator, db_path) catch |err| {
...
// 状态保存性能测试
fn benchmarkSaveStates(db: *AgentDatabase, allocator: std.mem.Allocator) !BenchmarkResult {
    ...
    for (0..num_operations) |i| {
        ...
        const state = AgentState.init(agent_id, session_id, StateType.context, test_data);
...
// 状态加载性能测试
fn benchmarkLoadStates(db: *AgentDatabase, allocator: std.mem.Allocator) !BenchmarkResult {
    const num_operations: u32 = 1000;
    ...
    for (0..num_operations) |i| {
        ...
        const state = try db.loadState(agent_id);
        if (state) |s| {
            allocator.free(s);
        }
    }
...
// 状态更新性能测试
fn benchmarkUpdateStates(db: *AgentDatabase, allocator: std.mem.Allocator) !BenchmarkResult {
    ...
    for (0..num_operations) |i| {
        ...
        try db.updateAgent(agent_id, updated_data);
    }
...
// 元数据操作性能测试
fn benchmarkMetadataOperations(db: *AgentDatabase, allocator: std.mem.Allocator) !BenchmarkResult {
    const num_operations: u32 = 1000;
    ...
    for (0..num_operations) |i| {
        ...
        // 模拟元数据操作（保存和加载状态）
        const state = AgentState.init(agent_id, @as(u64, i), StateType.context, value);
        try db.saveState(state);

        // 加载状态
        const loaded = try db.loadState(agent_id);
        if (loaded) |data| {
            allocator.free(data);
        }
    }
...
// 快照操作性能测试
fn benchmarkSnapshotOperations(db: *AgentDatabase, allocator: std.mem.Allocator) !BenchmarkResult {
    ...
    for (0..num_operations) |i| {
        ...
        // 模拟快照操作（保存和加载状态）
        const state = AgentState.init(agent_id, @as(u64, i), StateType.context, snapshot_name);
        try db.saveState(state);
...
// 批量操作性能测试
fn benchmarkBatchOperations(db: *AgentDatabase, allocator: std.mem.Allocator) !BenchmarkResult {
    ...
    for (0..num_batches) |batch| {
        ...
        for (0..batch_size) |i| {
            ...
            const state = AgentState.init(agent_id, session_id, StateType.working_memory, data);
            try db.saveState(state);
        }
    }
...
// 并发操作性能测试（模拟）
fn benchmarkConcurrentOperations(db: *AgentDatabase, allocator: std.mem.Allocator) !BenchmarkResult {
    const num_operations: u32 = 500;
    ...
    for (0..num_operations) |i| {
        ...
        if (i % 2 == 0) {
            try db.updateAgent(agent_id, "更新的并发数据");
        }
    }
...
// 内存使用测试
fn benchmarkMemoryUsage(db: *AgentDatabase, allocator: std.mem.Allocator) !void {
    ...
    for (0..num_states) |i| {
        ...
        const state = AgentState.init(agent_id, session_id, StateType.long_term_memory, large_data);
        try db.saveState(state);
    }
