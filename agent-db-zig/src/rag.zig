...
pub const RAGEngine = struct {
    ...
    pub fn hybridSearch(self: *Self, text_query: []const u8, query_embedding: []const f32, 
                       alpha: f32, limit: usize) !SearchResults {
        if (self.db == null) return error.DatabaseNotInitialized;
    ...
    pub fn buildContext(self: *Self, query: []const u8, search_results: []const SearchResult, 
                       max_tokens: usize) !RAGContext {
        _ = self;
