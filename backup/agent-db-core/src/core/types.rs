...
use uuid::Uuid;
use super::error::AgentDbError;

// 错误类型在 error.rs 中定义

// DatabaseConfig 已移动到 config.rs
...
impl Document {
    ...
    pub fn chunk_document(&mut self, chunk_size: usize, overlap: usize) -> Result<(), AgentDbError> {
        ...
        while pos < content_bytes.len() {
            ...
            let chunk_id = format!("{}_{}", self.doc_id, chunk_index);
            let chunk = DocumentChunk {
                id: chunk_id.clone(),  // 保持与 RAG 模块兼容
                chunk_id,
...
pub struct DocumentChunk {
    ...
    pub position: usize,
    pub size: usize,
    pub embedding: Option<Vec<f32>>,  // 添加嵌入向量支持
}

// 保持向后兼容的别名
pub type Chunk = DocumentChunk;

// 搜索结果
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SearchResult {
    pub document_id: String,  // 保持与 RAG 模块兼容
